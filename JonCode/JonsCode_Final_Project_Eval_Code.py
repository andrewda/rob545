...
class Car:
    ...
    def __init__(self):
        ...
        self.rewardModifier = {'green': 5, 'yellow': 1}
    ...
    def get_reward(self, game_map):
        ...
        y = int(self.center[1])
        new_reward = 0
        if game_map.get_at((x, y)) == UNSAFE_ZONE:
            # a='yellow'
            new_reward = self.distance / (CAR_SIZE_X / 2) * self.rewardModifier['yellow']
        elif game_map.get_at((x, y)) == SAFE_ZONE:
            # a='green'
            new_reward = self.distance / (CAR_SIZE_X / 2) * self.rewardModifier['green']
            new_reward = new_reward * lane_ratio + self.distance / (CAR_SIZE_X / 2)

        return new_reward
        # return (total_safe_time * lane_ratio) / (CAR_SIZE_X / 2) + (self.distance)/(CAR_SIZE_X / 2)
...
def run_training(genomes, config):
    ...
    while True:
        ...
        for i, car in enumerate(cars):
            ...
            # maybe change this so that it can select more than one option at once
            # ie. first choice is choice = output.index(max(output)) same as before
            # but you also calc how close output is to this max value choice
            # and based on the size of the difference increase probability second choice will be selected
            # ie. abs(choice-output) = 0, prob = 0
            # ie. abs(choice-output) = 0.25, prob = 0.25
            # ie. abs(choice-output) = 0.5, prob = 0.5
            # print(choice)
            if choice == 0:
                car.angle += 4  # Left
            elif choice == 1:
...
def run_testing(genomes, config, map):
    ...
    while True:
        ...
        # Display Info
        text = generation_font.render("Best Genome from EANN on Testing Track ", True, (0, 0, 0))
        text_rect = text.get_rect()
    ...
    return survived, still_alive, percent_on_green, dist_covered, fitness_value
...
def train_net(config_path, genome_path, gen_count):
    ...
    global s
    s = np.zeros([1, gen_count])[0]
    global b
    b = np.zeros([1, gen_count])[0]
    global s_i
    s_i = 0
...
if __name__ == "__main__":
    ...
    bdist = np.zeros((len(maps), max(gs)))



    #TODO: Run 10 training sets?
    genome_path = "Winner2.pkl"

    #train on a map
    tmap = maps[train_map_idx]
    # maps.remove(maps[train_map_idx])

    # print(survivors)
    # print(list(range(0, gen_count)))
    if ('all' in whattorun) or ('survivorgraph' in whattorun):
        gen_count = max(gs)
        # gen_count = 40
        # bdist = np.zeros((len(maps), gen_count))
        ...
        for ij in range(0, len(maps)):
            ...
            # global s
            # print(s)
            survivors[ij]= copy.copy(s)
            bdist[ij]= copy.copy(b)
            # print(survivors)

            plt.plot(copy.copy(s), label=maps[ij].split('.')[0])
        ...
        for k in range(0, len(maps)):
            plt.plot(bdist[k], label=maps[k].split('.')[0])
        plt.xlabel('Num of Generations')
    ...
    if ('all' in whattorun) or ('resultsTrainedBy1' in whattorun):
        #Setup Maps
        tmap = maps[train_map_idx]
        testingmaps = copy.copy(maps)
        testingmaps.remove(maps[train_map_idx])

        #Setup Empty Arrays
        testingdistances = np.zeros((len(testingmaps), n, len(training_gs)))

        # print(testingdistances)
        # sys.exit()
        #Repeat n times
        for ik in range(0, n):
            c = -1

            #for each generation count
            for gen_count in training_gs:
                c = c+1
                # train network
                train_net(config_path, genome_path, gen_count)
                ...
                #Evaluate on testing maps
                for a in range(0, len(testingmaps)):
                    config = neat.config.Config(neat.DefaultGenome,
                                                neat.DefaultReproduction,
                                                neat.DefaultSpeciesSet,
                                                neat.DefaultStagnation,
                    ...
                    survived, still_alive, percent_on_green, dist_covered, fitness_value = run_testing(genomes, config,
                                                                                                       testingmaps[a])
                    testingdistances[a][ik][c] = dist_covered
        ...
        plt.figure(1)
        for a in range(0, len(testingmaps)):
            testData = testingdistances[a]
            mean_testData = testData.mean(axis=0)
            max_testData = testData.max(axis=0)
            min_testData = testData.min(axis=0)
            std_testData = testData.std(axis=0)

            plt.errorbar(training_gs, mean_testData, std_testData, label=testingmaps[a].split('.')[0])
            plt.plot(training_gs, mean_testData)

        plt.xlabel('Num of Generations')
        plt.ylabel('Distance')
        plt.title('Generalized Distances Covered')
        plt.legend(loc='upper right')
        ...
        for a in range(0, len(testingmaps)):
            ...
            plt.plot(training_gs, mean_testData, label=testingmaps[a].split('.')[0])
        plt.xlabel('Num of Generations')
        plt.ylabel('Distance')
        plt.title('Average Distances Covered')
        plt.legend(loc='upper right')
    ...
    if ('all' in whattorun) or ('1train4test' in whattorun):
            ...
            for c in range(0, len(maps)):
                ...
                for j in range(0, len(maps)):
                    config = neat.config.Config(neat.DefaultGenome,
                                                ...
                                                neat.DefaultStagnation,
                                                config_path)
                    survived, still_alive, percent_on_green, dist_covered, fitness_value = run_testing(genomes, config, maps[j])
    ...
    if ('all' in whattorun) or ('DistancesAndGreenRatio' in whattorun):
        c = -1
        for gen_count in gs:
            c = c+1
            ress = np.zeros((len(maps), 4))

            train_net(config_path, genome_path, gen_count)
            # plt.plot(range(0, gen_count), s)
            # plt.xlabel('Num of Generations')
            ...
            for j in range(0, len(maps)):
                ...
                survived, still_alive, percent_on_green, dist_covered, fitness_value = run_testing(genomes, config, maps[j])
                print(survived, still_alive, percent_on_green, dist_covered, fitness_value)
                distances[j][c] = dist_covered
                greenrats[j][c] = percent_on_green
                # survivors[j][c] = still_alive

                # ress[j][0] = survived
                # ress[j][1] = percent_on_green
                # ress[j][2] = dist_covered
                # ress[j][3] = fitness_value
            if os.path.exists(genome_path):
                os.remove(genome_path)
        ...
        for k in range(0, len(distances)):
            plt.plot(gs, distances[k], label=maps[k].split('.')[0])
        plt.xlabel('Num of Generations')
        plt.ylabel('Distance')
        plt.title('Distances Covered')
        plt.legend(loc='upper right')
        plt.show()

        for k in range(0, len(greenrats)):
            plt.plot(gs, greenrats[k], label=maps[k].split('.')[0])
        plt.xlabel('Num of Generations')
        plt.ylabel('Ratio')
        plt.title('Ratio of Time Spent in Green Areas')
        plt.legend(loc='upper right')
        plt.show()
    ...
    # plt.show()
    print("Eval finished")
    # print(survived, percent_on_green, dist_covered)
    # print(ress)
    # print(hidden_nodes_count)
